...
static TIMER  gDelayTimer;
static BOOL   gbfTimeOut = FALSE;
...
{
    BOOL    bfResult = FALSE;
    BOOL    bfFirstDollar = TRUE;
    int     iZeroCount = 4;
    /* Open the serial port */
    hwOpenSCI3();
    /* Until the length of data is read OR the read is cancelled */
    while ((stLength)
    &&     (*pbfCancel == FALSE))
    {
        BYTE    byData;
...
/*****************************************************************************
 * Description: Function to set the time-out flag
 *
 * Parameters:
 *              N/A
 *
