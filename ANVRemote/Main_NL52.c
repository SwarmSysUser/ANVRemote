...
const BYTE gcpbyUpperKeyPress[] =
{
    /* Dollar a special sign to make TX wait */
    "\r\npress key upper, keyon\r\n$press key upper, keyoff\r\n*\r\n"
};

const BYTE gcpbyMenuKeyPress[] =
{
    /* Dollar a special sign to make TX wait */
    "\r\npress key menu, keyon\r\n$press key menu, keyoff\r\n*\r\n"
};
                                          ...
                                          char     *pszResponse)
{
    ...
    if (WriteSCI3(pbyCommand, stCommandLength, &gbfCancel))
    {
        ...
        /* Read the response */
        if (ReadSCI3(gpbyResponseBuffer,
...
static BOOL SendMenuKeyPress(void)
{
    /* Send on/off key-press */
    if (SCI3_SendCommandCheckResponse(gcpbyMenuKeyPress,
                                      sizeof(gcpbyMenuKeyPress),
                                      "R+0000"))
    {
        return TRUE;
    }
...
static BOOL StartManualRecord_Try(void)
{
                                      ...
                                      "Start"))
    {
                                          ...
                                          "Stop"))
        {
            /* Press the menu key */
            SendMenuKeyPress();
            /* Check that recording has been started */
            if (SCI3_SendCommandCheckResponse(gcpbyRecCheckMessage,
                                              sizeof(gcpbyRecCheckMessage),
                                              "Manual"))
            {
            ...
            /* Check again that recording has been started */
            if (SCI3_SendCommandCheckResponse(gcpbyRecCheckMessage,
                                              sizeof(gcpbyRecCheckMessage),
                                              "Manual"))
            {
...
static BOOL StartManualRecord(void)
{
    ...
    while (iCount--)
    {
        ...
        {
                                    /* Drop a marker */
            SCI3_SendCommandCheckResponse(gcpbyUpperKeyPress,
                                          sizeof(gcpbyUpperKeyPress),
                                          "R+0000");
            return TRUE;
        }
    }
